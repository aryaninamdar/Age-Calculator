...
new= tk.Label(root,bg="#D5C6FF")  #declaring a lable
...
def clean(entry_date,entry_month,entry_year):
    global new
    new.grid_forget()
    entry_date.delete(0,4)
    entry_month.delete(0,4)
    entry_year.delete(0,4)

#creating widgets such as labels,entry boxes and buttons and fixing its position onto window
title_label= tk.Label(root,text="AGE CALCULATOR",borderwidth=10,fg="#6600CC",bg="#D5C6FF")
title_label.config(font=("Broadway",29))
title_label.grid(row=0,column=0,columnspan=3)
label_date= tk.Label(root,text="BIRTH DATE : ",borderwidth=4,fg="#990099",bg="#D5C6FF")
label_date.config(font=("Arial Rounded MT Bold",15))
label_date.grid(row=1,column=0)
label_month= tk.Label(root,text="BIRTH MONTH : ",borderwidth=5,fg="#990099",bg="#D5C6FF")
label_month.config(font=("Arial Rounded MT Bold",15))
label_month.grid(row=2,column=0)
label_year= tk.Label(root,text="BIRTH YEAR : ",borderwidth=9,fg="#990099",bg="#D5C6FF")
label_year.config(font=("Arial Rounded MT Bold",15))
label_year.grid(row=3,column=0)

entry_date= tk.Entry(root,width=20,borderwidth=3)
entry_month= tk.Entry(root,width=20,borderwidth=3)
entry_year= tk.Entry(root,width=20,borderwidth=3)
...
submit= tk.Button(root,text="GET AGE!!",width=10,command=lambda:age(b_date,b_month,b_year),bg="#6600CC",fg="#D5C6FF",borderwidth=5)
submit.grid(row=4,column=0)

#calling clean function in button widget
clear= tk.Button(root,text="CLEAR",width=10,command=lambda:clean(entry_date,entry_month,entry_year),bg="#6600CC",fg="#D5C6FF",borderwidth=5)
